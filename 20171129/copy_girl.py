...
def url_jiexi(url_true):
    ...
    try:
        ...
        print(imag_num)
        print(imag_base)
        # noinspection PyPep8,PyPep8,PyRedundantParentheses
...
def save_image(content, title, i):  # 创建对应分类的文件夹，并且保存图片。
    ...
    print(dir_name)
    # noinspection PyPep8
    dir_path = 'G:/python/git/xiewei/20171115/images/{}'.format(dir_name)
    # noinspection PyBroadException
    try:
        os.mkdir(dir_path)
        print("创建文件夹成功")
...
def main():
    for i in range(start, end):
        ...
        if url_true:
            title, image_num, image_base = url_jiexi(url_true)
            # noinspection PyAssignmentToLoopOrWithParameter
            for i in range(1, int(image_num)):
                download_image(title, image_base, i)
        time.sleep(0.5)
